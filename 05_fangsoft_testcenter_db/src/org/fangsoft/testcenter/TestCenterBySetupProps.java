...
import org.fangsoft.testcenter.data.Setup;
...
public class TestCenterBySetupProps {
	public static void main(String[] args) {
		...
		Configuration.getPropsDaoFactory().getTestResultDao().save(tr);
...
//		trDao.save(tr);
	...
	public static void initPropsFiles() {
		Setup.exportCustomer2File();
		Setup.exportTest2File();
	}
	...
	public static Customer login() {
		...
		for (int i = 0; i < Configuration.MAX_LOGIN; i++) {
			...
			CustomerDao cdao = Configuration.getPropsDaoFactory().getCustomerDao();
...
//		QuestionArrayDao.getInstance().loadQuestion(test);
		Configuration.getPropsDaoFactory().getQuestionDao().loadQuestion(test);
		TestResult tr = newTestResult(customer, test);
		int count = 0;
...
//		TestDao tDao = TestArrayDao.getInstance();
		TestDao tdao = Configuration.getPropsDaoFactory().getTestDao();
