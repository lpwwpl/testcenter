...
public class TestCenterController implements ITestCenterController {
	...
	public void commitTest(TestResult testResult) {
		...
		testResult.setStatus(Status.FINISH);
		for(QuestionResult qr:testResult.getQuestionResults()) {
			if(qr.getAnswer()==null) {
				qr.setAnswer("");
			}
		}
	...
	public TestResult startTest(Test test, Customer customer) {
		...
		long theTestDeadTime = testResult.getStartTime().getTime()
				+ loadedTest.getTimeLimitMin() * 60 * 1000;
		testResult.setTestDeadTime(theTestDeadTime);
		this.testDeadTime = theTestDeadTime;
		System.out.println(loadedTest.getTimeLimitMin() * 60 * 1000+"\t"+testResult.getStartTime().getTime());
